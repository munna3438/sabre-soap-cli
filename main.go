...
func main() {
	...
	for {
		...
		fmt.Printf("Seat hold command : %s\n", seatHoldCommand)
		fmt.Println()
		...
		fmt.Printf("Blocking seat for booking class %s (%s -> %s, %s)...\n", in.BookingClass, in.From, in.To, in.Date)

		held := seatBlockLoop(cfg, sabreCfg, in, session, flightCommand, seatHoldCommand, li)
		if !held {
...
func postCancelMenu(sabreCfg *sabre.Config, session *sabre.SessionResult, li *lineInput) bool {
	for {
		fmt.Println()
		fmt.Println("Seat blocking stopped.")
...
func banner() string {
	return `
╔══════════════════════════════════════════╗
║          SABRE SEAT BLOCK v1.0           ║
║      Seat Hold Until Blocked Tool        ║
╚══════════════════════════════════════════╝
`
}
